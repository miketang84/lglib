...
>>Remarks: the relationship between Dict and table is try to implement the Dict API by table-extended methods and remove this inheritance relationship.
 
 
isDict()
for non-empty dict, true means Dict instance, false as non-Dict instance.
	
	Dict:keys()    -- return all the keys of Dict instance, but without keeping any order because of the feature of hash table.
	Dict:values()  -- return all the values of Dict instance, still without keeping any order
	Dict:hasKey()  -- check whether the specific key exist in the Dict object or not
	Dict:size()    -- return the length of Dict instance 
