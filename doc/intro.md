...
where `self` is the splited string, `delim` for delimiter, `count` for times that the delimiter could be replaced, `no_patterns` can be choosed as true|false|nil, which could turn off regular expression in delimiter or not.
...
every string generated by lua will be encoded in UTF8???  Lua can also handle UTF8 bytes directly
